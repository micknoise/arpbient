...
# open http://localhost:8000/arpbient/   (or `cd arpbient` and open http://localhost:8000/)
```
...
This app sits in `arpbient/` of the repo, alongside its horror sibling
**[Wraith](../wraith/)** in `wraith/`. The repo root has a landing page linking to both,
and a top-level [README](../README.md) describing the layout.

Older branches are Arpbient's development history:
...
- `v2-carpenter-sinister` — an experiment narrowing the harmony to a strict minor-key i/bVI
  cell and replacing the arp with bright stab hits. Kept for reference.
